...
use crate::functions::serialize::{
    serialize_song_folder_with_mapper,
    serialize_osu_db_with_mapper,
    open_osu_db,
};
use crate::functions::check::{ check_songs_dir, check_osu_db };
...
pub fn diff_sets(sets_old: &Beatmapsets, sets_new: &Beatmapsets) -> Beatmapsets {
    let diff_beatmapset_ids = diff_new_sets_by_sets(
        &sets_old.beatmapset_ids,
        &sets_new.beatmapset_ids
    );
    Beatmapsets {
...
pub fn diff_songs(songs_old: &SongsWithMapper, songs_new: &SongsWithMapper) -> SongsWithMapper {
    let diff_songs = diff_new_songs_by_song_id_with_mapper(
        &songs_old.songs,
        &songs_new.songs
    );
    SongsWithMapper {
